...
- **"Is there a field-measure visit open?" is `fieldMeasurePending()`, never a date compared to today** (ARCHITECTURE.md §8). The date is intent; the Field measurements component reading `approved` is the record. Three things fall out and each has a test: `fieldReq` doesn't gate it and confirming doesn't set it; the status picker shows even with Required off when a visit is scheduled; and a new, different `fieldDate` resets the status to pending in `applyItemPatch`.
...
- **What the exported delivery log prints is `deliveryLogRows(it)`, not `it.deliveries`.** The log and the stage are two different writers and only the first fills the array, so an item moved with the stage buttons registered nothing and the block stayed silent about material every other screen already showed as delivered. The function derives **one** row from the stage when there are no entries — never both, or the same material is counted twice — and skips OFCI. Any new surface that counts or shows "what arrived" goes through it.
- **Print margins are per-page, so they live in the report's repeated `<thead>` / `<tfoot>`.** The `@page` margin box has to stay at `0` (that band is where Chrome draws its own header/footer), and `.print-report`'s block padding lands only once, at the start and the end of the whole flow — page 2 opened flush against the paper edge. The whole report therefore sits inside a **sheet table** whose header group carries the branded block plus the top margin and whose footer group is a 9mm spacer; `.print-report` keeps only side padding. A new report block goes inside that table's `<tbody>` cell, not next to the table.
